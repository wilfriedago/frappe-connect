...
def on_document_event(doc, method: str):
	...
	"""
	# Skip during install/migrate/patch — tables may not exist yet
...
def get_matching_rules(doctype: str, event_name: str) -> list:
	"""Get all enabled hooks matching a DocType and event."""
...
def produce_message(
	...
	rule = frappe.get_doc("Connect Hook", rule_name)
	topic = rule.topic_override or settings.command_topic
	tenant_id = rule.tenant_id_override or settings.default_tenant_id

	# Resolve the actual schema name from the linked Connect Avro Schema
	schema_name = frappe.db.get_value("Connect Avro Schema", rule.avro_schema_name, "schema_name")
	if not schema_name:
		frappe.throw(f"Avro schema not found: {rule.avro_schema_name}")
	...
	log = ConnectMessageLog.log_produced(
		idempotency_key=idempotency_key,
	...
	try:
		...
		inner_schema = get_schema(schema_name, settings)
		...
		envelope = {
			"id": 0,
			"source": settings.producer_source_name or "openerp-connect",
