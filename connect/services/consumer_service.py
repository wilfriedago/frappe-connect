...
from connect.utils.idempotency import check_idempotency, generate_consumer_idempotency_key
...
def _process_message(msg, avro_deserializer, settings):
	...
	try:
		...
		if check_idempotency(idempotency_key):
			log_info("Consumer skipping duplicate", f"topic={topic} partition={partition} offset={offset}")
			return
		...
		if handler.condition:
			try:
				result = frappe.safe_eval(
					handler.condition,
					eval_globals={"payload": inner_payload, "envelope": envelope, "frappe": frappe},
				)
				if not result:
				...
				log.mark_failed(f"Handler condition eval failed: {e}", traceback.format_exc())
				frappe.db.commit()
				return
...
def _dispatch_sync_job(action, payload: dict, envelope: dict):
	...
	try:
		...
		log_error("Frappe Tweaks not installed", "Cannot enqueue Sync Job without Frappe Tweaks")
	except Exception as e:
...
def _dispatch_document_action(action, payload: dict, envelope: dict):
	...
	log_info("Document action enqueued", f"type={action.action_type}, doctype={action.target_doctype}")
