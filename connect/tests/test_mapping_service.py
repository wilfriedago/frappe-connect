...
class TestBuildPayload(unittest.TestCase):
	...
	def test_expression_source(self, mock_frappe):
		...
		mock_frappe.safe_eval = lambda expr, eval_globals=None: eval(expr, {"doc": eval_globals["doc"]})
		mapping = self._make_mapping(
			avro_field="fullName",
			avro_type="string",
	...
	def test_multiple_mappings(self, mock_frappe):
		...
		mappings = [
			...
			self._make_mapping(
				avro_field="name", avro_type="string", source_type="Field", source_field="customer_name"
			),
			self._make_mapping(
				avro_field="source", avro_type="string", source_type="Static", static_value="erpnext"
			),
		]
